...
// fields, recursing into Message via copyByName.
...
func (s *SignedBeaconBlock) FromView(view any) error {
	v, err := signedBeaconBlockVersion(view)
	if err != nil {
		return err
	}
...
// signedBeaconBlockVersion maps a SignedBeaconBlock view type to its DataVersion.
func signedBeaconBlockVersion(view any) (version.DataVersion, error) {
	switch view.(type) {
	...
	default:
		return version.DataVersionUnknown, fmt.Errorf("SignedBeaconBlock: unsupported view type %T", view)
	}
}
