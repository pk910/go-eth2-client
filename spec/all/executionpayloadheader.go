...
// ExecutionPayloadHeader is a fork-agnostic execution payload header containing
// the union of fields from every fork. Fields populated on a given instance
// depend on Version.
...
func (e *ExecutionPayloadHeader) viewType() (any, error) {
	switch e.Version {
...
func (e *ExecutionPayloadHeader) MarshalSSZDyn(ds sszutils.DynamicSpecs, buf []byte) ([]byte, error) {
	...
	if !ok {
		return nil, errors.New("ExecutionPayloadHeader: generated SSZ code missing")
	}

	fn := m.MarshalSSZDynView(view)
	if fn == nil {
		return nil, fmt.Errorf("ExecutionPayloadHeader: no view marshaler for version %d", e.Version)
	}
...
func (e *ExecutionPayloadHeader) UnmarshalSSZDyn(ds sszutils.DynamicSpecs, buf []byte) error {
	...
	if !ok {
		return errors.New("ExecutionPayloadHeader: generated SSZ code missing")
	}

	fn := u.UnmarshalSSZDynView(view)
	if fn == nil {
		return fmt.Errorf("ExecutionPayloadHeader: no view unmarshaler for version %d", e.Version)
	}
...
// populateVersion sets Version. ExecutionPayloadHeader has no nested
// versionable children.
func (e *ExecutionPayloadHeader) populateVersion(v version.DataVersion) {
	e.Version = v
}

// ToView returns a fresh fork-specific ExecutionPayloadHeader populated with
// e's fields.
...
func (e *ExecutionPayloadHeader) FromView(view any) error {
	...
	case *bellatrix.ExecutionPayloadHeader:
		...
		e.TransactionsRoot = v.TransactionsRoot
	...
	case *capella.ExecutionPayloadHeader:
		...
		e.BlockHash = v.BlockHash
		e.TransactionsRoot = v.TransactionsRoot
		e.WithdrawalsRoot = v.WithdrawalsRoot
	...
	case *deneb.ExecutionPayloadHeader:
		...
		return nil
	default:
		return fmt.Errorf("ExecutionPayloadHeader: unsupported view type %T", view)
	}
}
...
func (e *ExecutionPayloadHeader) HashTreeRootWithDyn(ds sszutils.DynamicSpecs, hh sszutils.HashWalker) error {
	...
	if fn == nil {
		return fmt.Errorf("ExecutionPayloadHeader: no view hasher for version %d", e.Version)
	}
...
func (e *ExecutionPayloadHeader) HashTreeRoot() (phase0.Root, error) {
...
// UnmarshalJSON delegates to the per-fork ExecutionPayloadHeader that matches Version.
...
// UnmarshalYAML delegates to the per-fork ExecutionPayloadHeader that matches Version.
// Caller must set Version before calling.
