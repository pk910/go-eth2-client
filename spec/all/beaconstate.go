...
// All field copies — including nested versionable children — go through
// copyByName, which walks dst fields by name and recurses into nested
// pointers/slices.
func (b *BeaconState) ToView() (any, error) {
	return toViewByCopy(b)
}

// FromView populates b from a fork-specific BeaconState. The view's concrete
// type determines Version (when not already pinned). All fields are copied
// via copyByName; populateVersion seeds Version on freshly-allocated nested
// children.
func (b *BeaconState) FromView(view any) error {
	v, err := beaconStateVersion(view)
	if err != nil {
		return err
	}
...
// beaconStateVersion maps a BeaconState view type to its DataVersion.
func beaconStateVersion(view any) (version.DataVersion, error) {
	switch view.(type) {
