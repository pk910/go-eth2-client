...
// Hash: 5e1a5b9110381bd1a93207ee539c050a38d7d6a48b6c3f665c0a7a15159acb10
...
func (t *BeaconState) UnmarshalSSZ(buf []byte) (err error) {
	...
	{ // Field #12 'Balances' (dynamic)
		...
		for i := range itemCount {
			buf := buf[8*i : 8*(i+1)]
			val8[i] = phase0.Gwei(binary.LittleEndian.Uint64(buf))
		}
		t.Balances = val8
	}
	{ // Field #15 'PreviousEpochParticipation' (dynamic)
		...
		for i := range itemCount {
			buf := buf[1*i : 1*(i+1)]
			val9[i] = altair.ParticipationFlags(buf[0])
		}
	...
	{ // Field #21 'InactivityScores' (dynamic)
		...
		for i := range itemCount {
			buf := buf[8*i : 8*(i+1)]
			val11[i] = binary.LittleEndian.Uint64(buf)
		}
