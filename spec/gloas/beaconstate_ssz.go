...
// Hash: 196beed311e1f401bc92b1725bba0e2bfaa4571947456fa92cc06eb669fd193f
...
func (t *BeaconState) UnmarshalSSZ(buf []byte) (err error) {
	...
	{ // Field #45 'PTCWindow' (static)
		...
		for i := range 96 {
			val8 := val7[i]
			buf := buf[4096*i : 4096*(i+1)]
			val8 = sszutils.ExpandSlice(val8, 512)
			for i := range 512 {
				buf := buf[8*i : 8*(i+1)]
				val8[i] = phase0.ValidatorIndex(binary.LittleEndian.Uint64(buf))
			}
			val7[i] = val8
		}
		t.PTCWindow = val7
	...
	{ // Field #12 'Balances' (dynamic)
		...
		for i := range itemCount {
			buf := buf[8*i : 8*(i+1)]
			val12[i] = phase0.Gwei(binary.LittleEndian.Uint64(buf))
		}
		t.Balances = val12
	}
	...
	{ // Field #16 'CurrentEpochParticipation' (dynamic)
		...
		for i := range itemCount {
			buf := buf[1*i : 1*(i+1)]
			val14[i] = altair.ParticipationFlags(buf[0])
		}
	...
	{ // Field #21 'InactivityScores' (dynamic)
		...
		for i := range itemCount {
			buf := buf[8*i : 8*(i+1)]
			val15[i] = binary.LittleEndian.Uint64(buf)
		}
	...
	{ // Field #27 'HistoricalSummaries' (dynamic)
		...
		for i := range itemCount {
			if val16[i] == nil {
				val16[i] = new(capella.HistoricalSummary)
			}
			buf := buf[64*i : 64*(i+1)]
	...
	{ // Field #34 'PendingDeposits' (dynamic)
		...
		for i := range itemCount {
			if val17[i] == nil {
				val17[i] = new(electra.PendingDeposit)
			}
	...
	{ // Field #35 'PendingPartialWithdrawals' (dynamic)
		...
		for i := range itemCount {
			if val18[i] == nil {
				val18[i] = new(electra.PendingPartialWithdrawal)
			}
			buf := buf[24*i : 24*(i+1)]
	...
	{ // Field #36 'PendingConsolidations' (dynamic)
		...
		for i := range itemCount {
			if val19[i] == nil {
				val19[i] = new(electra.PendingConsolidation)
			}
			buf := buf[16*i : 16*(i+1)]
	...
	{ // Field #42 'BuilderPendingWithdrawals' (dynamic)
		...
		for i := range itemCount {
			if val21[i] == nil {
				val21[i] = new(BuilderPendingWithdrawal)
			}
			buf := buf[36*i : 36*(i+1)]
			if err = val21[i].UnmarshalSSZ(buf); err != nil {
				return err
			}
		}
	...
	{ // Field #44 'PayloadExpectedWithdrawals' (dynamic)
		...
		for i := range itemCount {
			if val22[i] == nil {
				val22[i] = new(capella.Withdrawal)
			}
			buf := buf[44*i : 44*(i+1)]
			if err = val22[i].UnmarshalSSZ(buf); err != nil {
				return err
			}
		}
