...
// Hash: 4a69b6081e6d1362b93becea701fe996a4bdc313586f93d31ecd03933425a9ff
...
func (t *BeaconState) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	{ // Dynamic Field #24 'LatestExecutionPayloadBid'
	...
	{ // Dynamic Field #44 'PayloadExpectedWithdrawals'
		binary.LittleEndian.PutUint32(dst[offset44:], uint32(len(dst)-dstlen))
...
func (t *BeaconState) UnmarshalSSZ(buf []byte) (err error) {
	...
	// Field #24 'LatestExecutionPayloadBid' (offset)
	...
	offset42 := int(binary.LittleEndian.Uint32(buf[2741077:2741081]))
	if offset42 < offset38 || offset42 > buflen {
		return sszutils.ErrOffset
	}
	{ // Field #43 'LatestBlockHash' (static)
		buf := buf[2741081:2741113]
	...
	if offset44 < offset42 || offset44 > buflen {
		return sszutils.ErrOffset
	}
	{ // Field #7 'HistoricalRoots' (dynamic)
		buf := buf[offset7:offset9]
		val7 := t.HistoricalRoots
	...
	{ // Field #12 'Balances' (dynamic)
		...
		for i := range itemCount {
			buf := buf[8*i : 8*(i+1)]
			val10[i] = phase0.Gwei(binary.LittleEndian.Uint64(buf))
		}
		t.Balances = val10
	}
	{ // Field #15 'PreviousEpochParticipation' (dynamic)
		buf := buf[offset15:offset16]
		val11 := t.PreviousEpochParticipation
		itemCount := len(buf)
		...
		t.PreviousEpochParticipation = val11
	}
	{ // Field #16 'CurrentEpochParticipation' (dynamic)
		buf := buf[offset16:offset21]
		val12 := t.CurrentEpochParticipation
		itemCount := len(buf)
	...
	{ // Field #21 'InactivityScores' (dynamic)
		...
		for i := range itemCount {
			buf := buf[8*i : 8*(i+1)]
			val13[i] = binary.LittleEndian.Uint64(buf)
		}
		t.InactivityScores = val13
	}
	{ // Field #24 'LatestExecutionPayloadBid' (dynamic)
	...
	{ // Field #42 'BuilderPendingWithdrawals' (dynamic)
		buf := buf[offset42:offset44]
		val19 := t.BuilderPendingWithdrawals
		...
		for i := range itemCount {
			if val19[i] == nil {
				val19[i] = new(BuilderPendingWithdrawal)
			}
			buf := buf[36*i : 36*(i+1)]
		...
		t.BuilderPendingWithdrawals = val19
	}
	{ // Field #44 'PayloadExpectedWithdrawals' (dynamic)
		buf := buf[offset44:]
		val20 := t.PayloadExpectedWithdrawals
		...
		for i := range itemCount {
			if val20[i] == nil {
				val20[i] = new(capella.Withdrawal)
			}
			buf := buf[44*i : 44*(i+1)]
		...
		t.PayloadExpectedWithdrawals = val20
	}
	return nil
}
...
func (t *BeaconState) SizeSSZ() (size int) {
	...
	// Field #24 'LatestExecutionPayloadBid' offset (4 bytes)
