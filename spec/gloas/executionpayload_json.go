...
import (
	...
	"github.com/ethpandaops/go-eth2-client/codecs"
...
func (e *ExecutionPayload) UnmarshalJSON(input []byte) error {
	...
	switch {
	case bytes.Equal(raw["block_access_list"], []byte{'0', 'x'}), bytes.Equal(raw["block_access_list"], []byte{'0'}):
		// Empty.
	default:
		tmpBytes = bytes.TrimPrefix(bytes.Trim(raw["block_access_list"], `"`), []byte{'0', 'x'})
		if len(tmpBytes)%2 == 1 {
			tmpBytes = fmt.Appendf(nil, "0%s", string(tmpBytes))
		}
		...
		if len(tmp) > bellatrix.MaxBytesPerTransaction {
			return errors.New("block_access_list: incorrect length")
		}

		e.BlockAccessList = tmp
	}
